...
export class cborwebtoken {
    public async sign(payload: object, secret: string | Buffer): Promise<Buffer> {
        var mappedPayload = this.buildMap( payload );
...
const tokenResponse = cwt.sign(payload,secret ).then((tokenResponse) => {
...
var verifyTest = cwt.verify(token, Buffer.from(secret, 'hex')).then((verifyTest) => {
